    ...
    class TradeClaimsController < ApplicationController
      ...
      def create
        ...
        render json: {
          status:  "submitted",
          data:    tc,
          message: "Trade claim #{tc.reference} submitted — entering supervisory review queue."
        }, status: :created
      end
      ...
      def approve
        ...
        return render_terminal_state_error(claim) if claim.terminal?

        body  = JSON.parse(request.body.read) rescue {}
        notes = body["notes"] || body["decision_notes"] || ""

        # Step 1: compliance cleared
        claim.status               = "compliance_cleared"
        claim.compliance_cleared_at = Time.current
        claim.decision_notes       = notes if notes.present?

        # Step 2: automated corridor protocol verification (mutates claim in memory)
      ...
      def reject
        ...
        claim.update!(status: "rejected", rejected_at: Time.current, decision_notes: notes)
      ...
      # Governor escape hatch: re-run corridor check after liquidity injection.
      def re_evaluate
        claim = find_claim or return
      ...
      def request_clarification
        ...
        claim.update!(status: "clarification_requested", decision_notes: notes)
      ...
      def cancel
        ...
        claim.update!(status: "cancelled", cancelled_at: Time.current)
      ...
      def find_claim
        claim = TradeClaim.find_by(id: params[:id])
      ...
      def build_eligibility(claim)
        ...
        supervisor_ok = %w[compliance_cleared ready_to_mint insufficient_corridor_liquidity].include?(claim.status)
        {
          ...
          gates: {
            ...
            liquidity:           { passed: minting_ready,  note: minting_ready ? "Headroom confirmed and earmarked" : (claim.verification_note || "Not confirmed") },
